...
//    private float[] value=new float[]{-5.55f,-6.899f,-4.55f,-0.045f,21.511f,22.221f,22.330f,21.448f,21.955f,23.6612f,22,22.18883f,21.47995f};
    ...
    /**折线颜色*/
    private int mBrokenLineColor= getContext().getResources().getColor(R.color.colorlightblue);//Color.BLUE;
    /**折线文本颜色*/
    private int mBrokenLineTextColor= getContext().getResources().getColor(R.color.color13);;//Color.BLUE;
    /**折线宽度*/
    private float mBrokenLineWidth=2;
    /**折线文本大小*/
    private float mBrokenLineTextSize=15;
    /**折线圆的颜色*/
    private int mCircleColor= getContext().getResources().getColor(R.color.color13);;//Color.BLUE;
    /**计算后的x，y坐标*/
    ...
    /** * 将dip或dp值转换为px值，保证尺寸大小不变 * * @param dipValue * @return */
...
//    }
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        mPoints = getPoints(value,mNeedDrawHeight,mNeedDrawWidth,calculateValue,minValue,
                mBrokenLineLeft,mBrokenLineTop);
        /**绘制边框线和边框文本*/
    ...
    public void DrawLineCircle(Canvas canvas) {
        for (int i = 0; i <mPoints.length ; i++) {
            ...
            /** * drawCircle(float cx, float cy, float radius, Paint paint) * cx 中间x坐标 * xy 中间y坐标 * radius 圆的半径 * paint 绘制圆的画笔 * */
            canvas.drawCircle(point.x,point.y,radius,mCirclePaint);
        }
    }
    ...
    private void DrawBorderLineAndText(Canvas canvas) {
        /**对应的属性 * drawLine(float startX, float startY, float stopX, float stopY, Paint paint); * startX 开始的x坐标 * startY 开始的y坐标 * stopX 结束的x坐标 * stopY 结束的y坐标 * paint 绘制该线的画笔 * */
        /**绘制边框竖线*/
        //canvas.drawLine(mBrokenLineLeft,mBrokenLineTop-dip2px(5),mBrokenLineLeft,mViewHeight-mBrokenLineBottom,mBorderLinePaint);

        canvas.drawLine(mBrokenLineLeft, mBrokenLineTop-30, mBrokenLineLeft, mViewHeight-mBrokenLineBottom, mBorderLinePaint);
        canvas.drawLine(mBrokenLineLeft, mBrokenLineTop-30, mBrokenLineLeft - 10 , mBrokenLineTop-17, mBorderLinePaint);
        canvas.drawLine(mBrokenLineLeft, mBrokenLineTop-30, mBrokenLineLeft + 10, mBrokenLineTop-17, mBorderLinePaint);
...
//        canvas.drawLine(xPoint, margin / 6, xPoint + margin / 3, margin / 2, paint);
       ...
       // canvas.drawLine(mBrokenLineLeft,mViewHeight-mBrokenLineBottom,mViewWidth,mViewHeight-mBrokenLineBottom,mBorderLinePaint);
        ...
        canvas.drawLine(mBrokenLineLeft, mViewHeight-mBrokenLineBottom, mViewWidth, mViewHeight-mBrokenLineBottom, mBorderLinePaint);
        canvas.drawLine(mViewWidth, mViewHeight-mBrokenLineBottom, mViewWidth - 13, mViewHeight-mBrokenLineBottom - 8, mBorderLinePaint);
        canvas.drawLine(mViewWidth, mViewHeight-mBrokenLineBottom, mViewWidth - 13, mViewHeight-mBrokenLineBottom + 8, mBorderLinePaint);
...
//        canvas.drawLine(this.getWidth() - margin / 6, yPoint, this.getWidth() - margin / 2, yPoint + margin / 3, paint);
    ...
    /**根据值计算在该值的 x，y坐标*/
    public Point[] getPoints(float[] values, float height, float width, float max ,
                             float min, float left, float top) {
        float leftPading = width / (values.length-1);
        //绘制边距
