...
public class PortCardService extends Service  {
    ...
    public void onCreate() {
        ...
        myCom.setPort("/dev/ttyMT3"); //myCom.setPort("/dev/ttyS3");
...
//        }
    ...
    public void onEventMainThread(CardEvent response) {
        sendPortData(myCom,response.getMes(),response.getBys(),response.getComm());
    }
...
//                    });
    ...
    private void dispRecData(ComBean ComRecData){
        ...
        System.out.println("收到数据了"+ss.length);
        ...
        System.out.println("接收数据"+ss.toString());
        while(index != -1 && flag >= 500){
            command = 0x32;//命令
...
//                    | ((bys[index + 5] & 0xff) << 8) | ((bys[index + 4] & 0xff) << 0)))&0x0FFFFFFFFL;//校验码
                ...
                data = new byte[bys.length];
                System.arraycopy(bys,0,data,0,bys.length);
//                String msg = new String(data);
//                if(Utils.check_suml(data)== checkSum){
//                    System.out.println("接收数据校验正确"+Utils.check_suml(data)+"*"+checkSum);
//                }
//                System.out.println("接收数据校验2正确"+Utils.check_suml(data)+"*"+checkSum);
                EventBus.getDefault().post(new BleReadEvent("",command,data));
            bys = new byte[1000];
            flag = 0;
            break;
//                System.out.println("接收数据2"+msg+"*"+command);
